...
Explain that this approach helps ensure the doc works well when others read it (including when they use it with a coding agent). Ask if they want to try this workflow or prefer to work freeform.
...
- If images exist without alt-text, explain that when others use an agent to understand the doc, the agent won't be able to see them. Ask if they want alt-text generated. If so, request they paste each image into chat for descriptive alt-text generation.
...
**If no integrations are detected and in a web chat or app:** Suggest they can enable connectors in their chat settings to allow pulling context from messaging apps and document storage directly.

You may also use CLI tools like Atlassian's `acli` to pull in context from Jira, Confluence, or other Atlassian products, GitHub's `gh` to pull in context from GitHub, or other CLI tools to pull in context from other sources.
...
- If user mentions team channels or shared documents:
  ...
  - If integrations not available: Explain lack of access. Suggest they enable connectors in their chat settings, or paste the relevant content directly.

- If user mentions entities/projects that are unknown:
  - Ask if connected tools should be searched to learn more
  - Wait for user confirmation before searching

- As user provides context, track what's being learned and what's still unclear in a working note
...
**If access to sub-agents is available (e.g., in Cursor or similar):**
...
1. Open a fresh chat with a coding agent or AI assistant (e.g., in another tab or session)
2. Paste or share the document content (if using a shared doc platform with connectors enabled, provide the link)
3. Ask the reader agent the generated questions

For each question, have the reader agent provide:
- The answer
...
- If user seems frustrated: Acknowledge this is taking longer than expected and suggest ways to move faster. Review the conversation history to identify areas where you were misunderstanding the user's intent and adjust your process accordingly.
