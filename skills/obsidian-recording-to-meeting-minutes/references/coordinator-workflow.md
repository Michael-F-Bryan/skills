...
- Attendee frontmatter, existing human-written meeting notes, and any user hints.
...
- `scribe/merged.json` diarised transcript from the merged audio.
...
3. Concatenate recordings in note order into one scratch file (`/tmp/<slug>/audio/merged.*`) and run `scribe --format json` once.
4. Compute durations and chunk windows from the merged transcript. Prefer topic boundaries; split long meetings into 20–45 minute windows.
5. Build `manifest.json`. Include enough metadata for specialists to work without full coordinator context.
6. Dispatch speaker attribution first. Treat speaker IDs as globally consistent by default because diarisation came from one merged pass; record caveats only where evidence is mixed.
7. Dispatch transcript-polishing subagents per chunk with `delegate_task` (or the platform’s equivalent). Give each only the relevant excerpt paths, speaker-attribution output, style requirements, and output path. Require the specialist to apply `/transcript-polisher` in strict Obsidian mode.
...
11. Dispatch QA/audit on the final draft and evidence manifest. QA is the release gate; do not replace this with only a local script.
...
1. Complete steps 1–5 as usual (vault rules, embeds, merged transcription, manifest, chunk excerpts on disk).
...
3. **Polishing:** For each chunk, follow `references/transcript-polishing-agent.md` and explicitly apply `/transcript-polisher` with inputs limited to that chunk’s excerpt files + speaker output; write `agent-outputs/polished-chunk-NN.md`.
...
- Ignoring attendee/frontmatter/human note evidence when mapping speakers.
