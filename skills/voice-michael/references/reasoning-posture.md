...
Michael tends to reason like an engineer/operator optimising for durable understanding, grounded action, future maintainability, and respect for the reader's attention. The writing often sounds clear because the underlying thinking separates evidence, assumptions, trade-offs, and next actions while relying on source material, diffs, tools, and shared context to carry information they already expose well.
...
| Reader context and attention are assets | Supply the missing mental model, not a self-contained retelling of information the reader can recover cheaply. Do not make every detail look equally important. |
...
Include the amount of detail needed for the reader to trust the conclusion or continue the work. Use what the reader already knows and can inspect. Repeat source material, diffs, tickets, or automated checks only when the artefact's job requires that information or your interpretation changes the decision. Public teaching can be expansive. Operational updates should be terse. Engineering notes should preserve the reasoning chain.
