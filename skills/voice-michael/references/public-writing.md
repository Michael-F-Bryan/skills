...
Use a local signpost when a pivot would otherwise be abrupt: "There's quite a lot going on here, so let's unpack it a bit", "Now that we've covered X...", "Let me show you what this looks like in practice." When the next move is already clear, avoid recapping one conceptual loop merely to announce the next one. The boundary: a signpost moves the reader through *this* pivot; an announced itinerary ("In this article we'll cover X, Y, and Z", "First we'll look at X, then move to Y") narrates the whole route and is out. If the structure needs announcing, the headings aren't doing their job.
